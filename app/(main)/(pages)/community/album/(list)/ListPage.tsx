...
export default function ListPage({ size, currPage }: ISearchParamsInfo) {
  const { data: { list, count } = { list: [], count: 0 }, isLoading } = useSelectPageList<AlbumRow>("albums", size, currPage, "show", { filter: "id", sort: "desc" });

  console.log(size, currPage);

  return (
    <div className="inner">
      {isLoading ? <StateView text="로딩중" /> : list.length <= 0 ? <StateView text="게시물 없음" /> : <PhotoBoard list={list} variant="album" />}
